...
const Gallery = () => {
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-[200px] md:h-[220px] my-6 mb-12 mx-auto max-w-[340px] md:max-w-[600px]"
    >
      {imageOrder.map((orderIndex, i) => {
        ...
        return (
          <motion.div
            key={orderIndex}
            ref={(el) => {
              imageRefs.current[orderIndex] = el
            }}
            className="gallery-image absolute cursor-pointer"
            style={{
              rotate:
                isMobile && activeImageIndex === orderIndex
                  ? "0deg"
                  : image.rotate,
              zIndex:
                isMobile && activeImageIndex === orderIndex
              ...
              transition:
                isDragging && draggedIndex === orderIndex
                  ? "none"
                  : "transform 0.3s ease, rotate 0.3s ease, top 0.3s ease, left 0.3s ease",
            }}
