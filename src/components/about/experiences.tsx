    ...
    <div className="mt-8 mb-12">
      ...
      <div className="space-y-6 md:space-y-6 space-y-0">
        {experiences.map((experience, index) => (
          ...
          >
            {/* Timeline dot */}
            <div className={`absolute left-0 top-1.5 w-2 h-2 rounded-full bg-black dark:bg-white transition-transform duration-300 ${(isMobile && activeExpIndex === index) || (!isMobile && 'group-hover:scale-150') ? 'scale-150' : ''}`}></div>
            
            {/* Content */}
            <div className="pb-4">
              <div className="flex flex-col md:flex-row md:items-baseline md:justify-between mb-1">
                <h3 className={`font-medium text-sm transition-all duration-300 ${(isMobile && activeExpIndex === index) || (!isMobile && 'group-hover:tracking-wide') ? 'tracking-wide' : 'tracking-tight'}`}>{experience.company}</h3>
              ...
              <div className="flex">
                <span className={`mr-2 mt-1.5 rounded-full bg-black dark:bg-white transition-all duration-300 ${(isMobile && activeExpIndex === index) || (!isMobile && 'group-hover:w-1.5 group-hover:h-1.5') ? 'w-1.5 h-1.5' : 'w-1 h-1'}`}></span>
                <p className={`text-xs font-light transition-opacity duration-300 ${(isMobile && activeExpIndex === index) || (!isMobile && 'group-hover:opacity-100') ? 'opacity-100' : 'opacity-80'}`}>{experience.description}</p>
              </div>
            </div>
          </motion.div>
        ))}
